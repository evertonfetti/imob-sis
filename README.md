...
- **Conectar:** *Redes sociais → Contas conectadas → Entrar com o Facebook*. O sistema lista as Páginas e as contas do Instagram profissional ligadas a elas, e você escolhe quais usar. Cada **usuário** cadastra os próprios apps da Meta na mesma tela (nome + ID + chave secreta, validados na Meta e guardados criptografados no banco), sem tocar no servidor, e escolhe por qual app entrar. **Apps e contas conectadas são privados** de quem os cadastrou: os colegas só veem e publicam se o dono marcar *Compartilhar com a equipe* (quem administra a empresa gerencia o que foi compartilhado); a tela mostra o passo a passo e a URI de redirecionamento a cadastrar. `META_APP_ID`/`META_APP_SECRET` são só um padrão opcional para empresas que não cadastraram o seu. Em modo de desenvolvimento só quem tem função no app consegue entrar; para liberar a todos é preciso enviar o app para a **revisão da Meta**.
...
- **Painel Mídia / IA (menu):** visão geral das fotos (publicáveis, editadas por IA, com falha), imóveis publicados que precisam de atenção (sem foto ou com menos de 5), estado da marca d'água, consumo de IA do mês e o histórico das últimas edições (modelo, custo, quem pediu, se está publicada). A edição em si acontece na galeria de cada imóvel.
...
- **Contas de IA (Empresa → Imagens e IA):** cadastre quantas contas quiser (**OpenAI, Google Gemini, Anthropic (Claude) e Groq**; inclusive várias do mesmo provedor). OpenAI e Gemini servem para texto e para editar imagens; Claude e Groq são só de texto (agente de atendimento e textos), então seus modelos nunca aparecem nas opções de edição de fotos. Ao informar a chave, o sistema **lista os modelos que ela dá acesso**, separados por finalidade (*editar imagens*, *gerar texto e conversar*, outros) e você marca os que quer usar, com **nível** (econômico, padrão, premium) e **custo estimado** editáveis. Dá para trocar a chave, pausar a conta, ativar/desativar modelos e buscar modelos novos depois. Chaves ficam criptografadas e nunca voltam para a tela; o **modo básico** (no servidor, sem chave e sem custo) só melhora foto e iluminação.
- **Modelo por edição:** há um padrão geral e um padrão **por tipo de edição** (ex.: modelo econômico para melhorar foto, premium para decorar); quem edita pode trocar o modelo na hora. Limite mensal de edições pagas, consumo por modelo, uma edição por foto por vez e sem nova tentativa automática (cada tentativa paga custa).
...
### Agente de atendimento por IA (WhatsApp)

- **O que faz:** responde os clientes que escrevem no WhatsApp na hora, apresenta-se como assistente virtual, conversa sobre o imóvel, **envia fotos** (só de imóveis publicados), tira dúvidas iniciais e vai registrando no lead o que o cliente conta (orçamento, venda/aluguel, região, dormitórios, prazo — o que também alimenta score e sugestão de imóveis). Pedido de visita vira tarefa para o corretor.
- **Onde aprende:** dos **imóveis do sistema** (dados oficiais; valor mínimo de negociação e dados do proprietário nunca vão para a IA) e de **documentos** enviados (PDF, DOCX, TXT, MD, CSV): por exemplo, o guia de financiamento. Os documentos são divididos em trechos e consultados por busca de texto (sem acento) a cada pergunta.
- **Transferência para pessoa:** quando o cliente pede um atendente/corretor (decisão imediata, sem gastar IA), quando o assistente não sabe responder, em negociação/reclamação, no limite de respostas por conversa ou se a IA ficar indisponível (o cliente recebe um aviso e não fica sem resposta). O corretor recebe uma **tarefa urgente** e a timeline registra. Com a conversa nas mãos de uma pessoa, **o robô não interage mais**.
- **Devolver ao robô:** em *Conversas* há *Assumir*, *Devolver ao robô* e *Finalizar*. Ao devolver, o assistente volta a responder **na próxima mensagem** do cliente (não puxa assunto). Uma pessoa que responde pelo painel assume a conversa sozinha; conversa **finalizada** reabre com o assistente quando o cliente escreve de novo. Também voltam ao assistente: conversas de leads **fechados ou perdidos** (finalizadas automaticamente; o cliente que volta entra como lead novo) e, se configurado, conversas com uma pessoa **paradas há X horas** (*Devolver ao assistente após inatividade*).
- **Configuração (Empresa → Agente de atendimento):** modelo de texto (de qualquer conta cadastrada), nome, orientações da imobiliária, mensagem de transferência, limites (respostas, fotos, espera), **base de conhecimento**, **teste sem enviar nada** (mostra a resposta, as fontes consultadas, ações e custo) e atividade recente com o custo de cada resposta.
- **Segurança:** o modelo só "pede" ações; o servidor valida cada uma (foto só de imóvel público da empresa, campos do lead limitados e tipados, sem apagar nem mudar etapa). Instruções do cliente para "ignorar regras" são ignoradas; nunca inventar preço, prazo ou promessa de crédito. Uma pessoa que assume enquanto a IA pensa faz a resposta ser descartada.
