...
import type { SocialAccountDto } from '@imob/types';
...
export class SocialAccountsService {
  ...
  get configured() { return !!(this.env.META_APP_ID && this.env.META_APP_SECRET); }
  ...
  graph() {
    return new SocialGraph({
      ...
      appId: this.env.META_APP_ID ?? '', appSecret: this.env.META_APP_SECRET ?? '', pollMs: this.env.SOCIAL_POLL_MS,
    });
  }
  ...
  connectUrl(ctx: AuthedCtx) {
    if (!this.configured) throw new AppException('SOCIAL_NOT_CONFIGURED', 409);
    const state = this.sign({ cid: ctx.user.companyId, uid: ctx.user.id, exp: Date.now() + STATE_TTL_MS, n: randomBytes(8).toString('hex') });
    return { url: this.graph().oauthDialogUrl({ redirectUri: this.redirectUri, state, scopes: SOCIAL_SCOPES }) };
  }
  ...
  async check(ctx: AuthedCtx, id: string) {
    ...
    if (!a) throw notFound('Conta não encontrada.');
    try {
      await this.graph().pageName(a.pageId ?? a.externalId, this.token(a.secrets));
