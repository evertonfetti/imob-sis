...
export class IntelligenceScheduler implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(@Inject(ENV) private readonly env: Env, private readonly prisma: PrismaService, private readonly score: LeadScoreService, private readonly tasks: TasksService, private readonly settings: IntelligenceSettingsService) {}
  ...
  /** Lead em aberto, com responsável, parado há mais de N dias (N é da empresa): uma tarefa de retomada por "período parado" (não repete a cada ciclo). */
  async staleLeads(now = new Date()): Promise<number> {
    let created = 0;
    for (const { id: companyId } of await this.prisma.company.findMany({ where: { status: 'ACTIVE' }, select: { id: true } })) {
      const cfg = await this.settings.get(companyId);
      if (!cfg.autoStaleTasks) continue;
      const first = await this.prisma.pipelineStage.findMany({ where: { position: 0, pipeline: { companyId } }, select: { id: true } });
      const leads = await this.prisma.lead.findMany({
        where: { companyId, status: { in: ['NEW', 'CONTACTED', 'QUALIFIED'] }, brokerId: { not: null }, stageEnteredAt: { lt: new Date(now.getTime() - cfg.staleDays * DAY) }, ...(first.length && { stageId: { notIn: first.map((s) => s.id) } }) },
