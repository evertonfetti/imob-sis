...
export class MediaQueue implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    this.worker = new Worker(QUEUE, (job) => this.processor.process(job.data.mediaId, { force: !!job.data.force }), { connection: this.connection, prefix, concurrency: 2 });
  ...
  async enqueue(mediaId: string, opts: { force?: boolean } = {}) {
    if (!this.queue) {
      try { await this.processor.process(mediaId, opts); } catch (e) { await this.processor.markFailed(mediaId, e); }
      return;
    }
    try {
      // Sem resposta do Redis em 3s, não deixamos o upload pendurado: processa em linha.
      await Promise.race([
        this.queue.add('process', { mediaId, force: !!opts.force }, { attempts: ATTEMPTS, backoff: { type: 'exponential', delay: 3000 }, removeOnComplete: 200, removeOnFail: 500 }),
      ...
      try { await this.processor.process(mediaId, opts); } catch (err) { await this.processor.markFailed(mediaId, err); }
    }
  }
